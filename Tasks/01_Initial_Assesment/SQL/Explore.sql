...
-- Total Records
SELECT 'Dish Count: ' || Count(*) AS cnt 
FROM Dish
;
-- 426985

-- uniqueness of PK
SELECT 'Dish with non-unique PK' || cnt
FROM
(
SELECT  Count(*) AS cnt
FROM Dish 
...
SELECT 'Dish Name Max Length: ' || MAX(Length(name)) Max_Len
FROM Dish
;
-- 1387 chars;
...
SELECT 
        'Unique Dish lower(name) w/o Extra spaces',
        Count(Id) AS Row_Count,
...
select 'Dishes to merge based on same name: ' || Count(*)
FROM
(
...
SELECT 'Mismatch Dish names: ' || Count(*)
FROM
(
SELECT 
        Id,
...
SELECT 
        'Unique Dish Names w/o Extra quotes',
        Count(Id) AS Row_Count,
...
SELECT 'Mismatch Dish names ignoring quotes: ' || Count(*)
FROM
(
SELECT 
        id,name,
...
SELECT 
        'Cleansed Dish Name Max Length: ' || max(length(REPLACE(REPLACE(trim(lower(name)),'"',''),'''',''))) As Dish_Name_Count
FROM Dish
;
-- 1377 Chars

SELECT 
        'Cleansed Dish Name Count: ' || count(distinct REPLACE(REPLACE(REPLACE(trim(lower(name)),'"',''),'''',''),'&','and')) As Dish_Name_Count
FROM Dish
;
-- Dish_Name_Count
-- 391424

SELECT 'Dishes to merge after name cleansing: ' || Count(*)
FROM
(
SELECT 
